...
test('decision ledger separates editorial pending from migration ready', () => {
  ...
  const decisions = {
    decisions: {
      'editorial:en-mix': {
        status: 'EDITORIAL_REWRITE_REQUIRED',
        strategy: 'REGENERATE_SECTION',
        reviewedAt: '2026-09-14',
        reason: 'Semantic condensation.'
      },
      'safe:en-mix': {
        status: 'SAFE_STRUCTURE_ONLY',
        strategy: 'WHITESPACE_ONLY',
        reviewedAt: '2026-09-14',
        reason: 'Only paragraph boundaries differ.'
      }
    }
  };
...
test('review priority rewards localized, recent, lower-delta candidates', () => {
  ...
  const a = payload.queue.find(row => row.id === 'localized');
  const b = payload.queue.find(row => row.id === 'diffuse');
  assert.ok(a.reviewPriority > b.reviewPriority);
});
