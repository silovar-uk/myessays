...
1. Current content contract and runtime behavior (`scripts/content-contract.mjs`, `scripts/build-content-index.mjs`, `reader-versions.js`, `library-versions.js`, `index.html`).
...
- Canonical article membership is discovered from `essays/*.md`; `data/index.json` is a generated runtime index and must not be hand-edited.
...
- Markdown files and their frontmatter are the membership source of truth.
- `data/index.json` and `data/versions-index.json` are generated by `node scripts/build-content-index.mjs --write`.
- `node scripts/build-content-index.mjs --check` must pass before publication.
- Missing files, duplicate canonical IDs, orphan derived versions and duplicate Reading Modes are content-contract errors.
- The canonical runtime loader is fail-soft: one broken article must not prevent other valid articles from loading.
