...
# 1) app.js: route identity comes only from the shared hash route parser.
app = read('app.js')
app = replace_once(
    app,
...
# 2) All root reader runtimes stop parsing essay identity independently.
direct_parser = """const match = location.hash.match(/^#\\/essay\\/(.+)$/);\n    if (!match) return '';\n    try { return decodeURIComponent(match[1]); }\n    catch { return match[1]; }"""
shared_parser = "return window.MyEssaysRoute?.parse?.().articleId || '';"
for filename in ['reader-versions.js', 'reader-language-instant.js', 'reading-locators.js', 'reader-reading-pivot.js', 'reader-v2.js']:
    text = read(filename)
    if direct_parser not in text:
        raise SystemExit(f'{filename}: direct essay hash parser not found')
    text = text.replace(direct_parser, shared_parser)
    write(filename, text)

# 3) reader-versions: an explicit URL language wins over saved preference.
...
versions = replace_once(
    ...
    'reader-versions desired priority'
)
write('reader-versions.js', versions)

# 4) Instant Controller owns URL <-> desired Reading Mode synchronization.
...
instant = replace_once(
    ...
    'instant render route sync'
)
instant = replace_once(
    instant,
    """    renderIntent,\n    syncControl: syncPersistentControl\n  });""",
    """    renderIntent,\n    syncControl: syncPersistentControl,\n    syncFromRoute\n  });""",
    'instant expose route sync'
)
instant = replace_once(
    instant,
    """        schedulePreload();\n        schedulePersistentControl();\n      }, { once: true })\n    : (() => {\n        schedulePreload();\n        schedulePersistentControl();\n      })();""",
...
# 5) Static ownership + cache tests.
...
# 6) index.html: route state before app + cache bust all changed runtimes.
html = read('index.html')
html = replace_once(
    html,
    '  <script src="app.js?v=20260829-2045" defer></script>',
    '  <script src="route-state.js?v=20260910-1" defer></script>\n  <script src="app.js?v=20260910-1" defer></script>',
    'index route state'
)
cache_updates = {
...
for old, new in cache_updates.items():
    if old not in html:
        raise SystemExit(f'index cache key not found: {old}')
    html = html.replace(old, new, 1)
write('index.html', html)

# 7) Browser QA dedicated to URL <-> Reading Mode synchronization.
qa = r'''const { chromium } = require('playwright');
...
async function openCase(browser, { id, lang, preferred, expectedVersion, expectedLang }) {
  ...
  if (preferred) {
    await context.addInitScript(value => localStorage.setItem('myessays:reader-language', value), preferred);
  }
...
(async () => {
  ...
  try {
    ...
    // Explicit URL beats a conflicting saved preference.
    {
      const { context } = await openCase(browser, { id: THREE, lang: 'ja', preferred: 'en-mix', expectedVersion: 'ja', expectedLang: 'ja' });
      await context.close();
    }

    // Legacy no-lang entry keeps saved preference, then becomes a canonical shareable URL.
    {
      const { context } = await openCase(browser, { id: THREE, preferred: 'en-mix', expectedVersion: 'en-mix', expectedLang: 'en' });
      await context.close();
    }

    // Invalid or unavailable public languages normalize to JA and the URL follows actual state.
    {
      const { context } = await openCase(browser, { id: THREE, lang: 'foo', expectedVersion: 'ja', expectedLang: 'ja' });
      await context.close();
    }
    {
      const { context } = await openCase(browser, { id: TWO, lang: 'es', expectedVersion: 'ja', expectedLang: 'ja' });
      await context.close();
    }

    // Direct controls update the URL immediately; rapid intent still ends at the latest mode.
    {
      ...
      // A reload/share of the resulting URL restores the same language.
      await page.reload({ waitUntil: 'domcontentloaded' });
      await page.waitForFunction(() => window.MyEssaysReaderVersions?.currentVersion?.() === 'es-mix'
        && window.MyEssaysRoute?.parse?.().lang === 'es', null, { timeout: 10000 });
      await context.close();
    }
...
'''
write('scripts/reading-mode-url-qa.cjs', qa)
...
print('Route state migration applied.')
