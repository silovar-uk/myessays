...
title: "あなたの組織図は、もう半分プロダクトの設計図になっている"
subtitle: "コンウェイの法則を、会議・Slack・マイクロサービスから本気で考える"
created: "2026-09-20"
updated: "2026-09-20"
type: "Essay"
status: "published"
...
grow: true
abstract: "なぜ組織を分けると、システムまで似た境界で割れやすいのか。1968年のMelvin Conwayの原論文に戻り、企業・産業研究、Windows Vistaの事例、142研究のレビュー、Inverse Conway Maneuverまで追う。コンウェイの法則を「組織図がコードに写る」という標語ではなく、通信経路が設計可能性そのものを狭める現象として捉え直し、実務で組織とシステムの境界を監査する方法まで落とし込む。"
---

# あなたの組織図は、もう半分プロダクトの設計図になっている
## コンウェイの法則を、会議・Slack・マイクロサービスから本気で考える

会社のSlackを眺めていると、たまに「このチャンネル構成、そのままシステム構成図では？」と思う瞬間がある。
...
調べる前、私はこの法則を「組織図どおりにソフトウェアが分割される」という有名な小話だと思っていた。調べた後は、少し違って見える。**怖いのは成果物が組織に似ることではない。組織を決めた時点で、設計者が探索しやすい解そのものが偏ることだ。**
...
ここで重要なのは、コピーされるのが正式な組織図そのものではなく、**communication structure、つまり誰と誰が実際に意思疎通できるかという構造**だという点である。
...
これは「部署AがモジュールAを作る」という結果の話だけではない。
...
だから組織は、意識的にも無意識にも、**「よく会話する人たちの中に、強い技術的依存を閉じ込める」**方向へ寄りやすい。

これは設計の美学というより、日々の面倒くささに対する適応である。
...
この話はあまりに説明力が高いので、何でも後付けで説明できてしまう危険がある。
...
つまり、**mirroringは広く見られる。しかし普遍的ではない。**
...
ただし、ここで「マイクロサービスが欲しい。よし、30チームに分けよう」と走ると、急にコメディになる。
...
コンウェイの法則を実務へ持ち込むとき、一番危ない誤読は「チームを分ければ、きれいに疎結合になる」である。
...
Conwayの原論文はsoftwareだけに限定せず、systemsを設計する組織一般を扱っている。
...
実務で使うなら、組織図とアーキテクチャ図を並べるだけでは足りない。
...
研究を広げると、そのmirroringはかなり頻繁に観察されるが、普遍ではない。デジタルツール、共有規約、モジュール設計、組織横断の関係によって鏡は壊せる。場合によっては、壊した方がいい。

そして逆コンウェイまで行くと、見え方が反転する。
...
以下は、本稿の調査を実務で再利用するための診断プロンプトである。
