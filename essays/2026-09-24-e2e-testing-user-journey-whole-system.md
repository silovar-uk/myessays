...
自動実行基盤のギットハブ・アクションズ（GitHub Actions）にある「画面品質確認（Visual QA）」という処理は、ローカルにWebサーバーを立て、プレイライトとクロミウムを導入してから複数のブラウザ検証を実行する。その中の `scripts/visual-qa.mjs` は、デスクトップとスマートフォン相当の画面幅で記事を開き、本文が表示されたか、日本語版から英語混じり版へ切り替えられたか、横方向にはみ出していないか、ブラウザ上でエラーが出ていないかを確認し、画面画像も保存する。
