(() => {
  ...
  function findByPair(root, pairId) {
    return pairs()?.findByPairId?.(root, pairId) || [...(root?.querySelectorAll?.('[data-pair-id]') || [])].find(el => el.dataset.pairId === pairId) || null;
  }

  function ensureModeBar() {
    let bar = content.querySelector('.reader-mode-bar');
    if (!bar) {
    ...
    const buttons = ['ja', ...available].map(version => {
      ...
      return `<button type="button" class="reader-mode-button${active ? ' is-active' : ''}" data-reader-mode-version="${version}" aria-pressed="${active}">${d.label}</button>`;
    }).join('');
    const compare = available.length
      ? `<button type="button" class="reader-mode-button reader-mode-compare${compareMode ? ' is-active' : ''}" data-reader-mode-compare aria-pressed="${compareMode}">比較</button>`
      : '';
  ...
  function setActiveBlock(block) {
    if (!block?.dataset?.pairId || block.closest('.reader-mode-bar,.reader-compare-view')) return;
    if (activeBlock && activeBlock !== block) activeBlock.classList.remove('is-language-active-paragraph');
    activeBlock = block;
    activePairId = block.dataset.pairId;
  ...
  async function openLens(block) {
    if (!block?.dataset?.pairId || compareMode) return;
    setActiveBlock(block);
    const panel = ensureLens();
    const target = targetVersion();
    if (!target) return;
    panel.dataset.pairId = block.dataset.pairId;
    ...
    const counterpart = await counterpartFor(block.dataset.pairId, target);
    if (panel.dataset.pairId !== block.dataset.pairId) return;
    panel.querySelector('.language-lens-counterpart-copy').textContent = counterpart?.textContent?.trim() || '';
    panel.querySelector('.language-lens-status').textContent = counterpart ? '同じ段落' : 'この段落は対応版なし';
    ...
    const blocks = pairs()?.eligibleBlocks?.(content)?.filter(el => el.dataset.pairId) || [];
  ...
  function moveLens(delta) {
    const blocks = pairs()?.eligibleBlocks?.(content)?.filter(el => el.dataset.pairId) || [];
    const index = blocks.findIndex(el => el.dataset.pairId === ensureLens().dataset.pairId);
  ...
  async function toggleFlip() {
    ...
    const block = pairs()?.findByPairId?.(content, pairId);
    if (!block) return;
  ...
  async function enterCompare(target = '') {
    ...
    [...content.children].forEach(child => {
      if (!child.matches('.reading-stats,.reader-mode-bar,.reader-compare-view')) child.classList.add('language-source-hidden');
    });
    ...
    const jaBlocks = pairs()?.eligibleBlocks?.(jaRoot)?.filter(el => el.dataset.pairId) || [];
    const targetDef = definition(target);
    ...
    const rows = jaBlocks.map(ja => {
      const alt = findByPair(altRoot, ja.dataset.pairId);
      const heading = /^H[23]$/.test(ja.tagName);
      return `<div class="reader-compare-row${heading ? ' is-heading' : ''}" data-pair-id="${ja.dataset.pairId}"><div class="reader-compare-cell" lang="ja"><span>JA</span><div>${ja.innerHTML}</div></div><div class="reader-compare-cell" lang="${targetDef.lang || 'en'}"><span>${targetDef.badge}</span><div>${alt ? alt.innerHTML : '<em>対応なし</em>'}</div></div></div>`;
    }).join('');
  ...
  content.addEventListener('pointerover', event => {
    ...
    const block = event.target.closest?.('[data-pair-id]');
  ...
  content.addEventListener('click', event => {
    ...
    if (event.target.closest?.('[data-reader-mode-compare]')) {
      enterCompare();
      return;
    }
  ...
  window.addEventListener('resize', positionAffordance);
